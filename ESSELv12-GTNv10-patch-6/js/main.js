...
function renderDbTables() {
    ...
    if (!state.appData.openDbSections) state.appData.openDbSections = {};
    ...
    const createSection = (title, dbKey = null) => {
        ...
        titleContainer.innerHTML = `<svg class="w-5 h-5 text-gray-500 transform transition-transform duration-200 accordion-icon" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"></path></svg><h4 class="font-bold text-lg text-gray-800 dark:text-gray-100">${escapeHtml(title)}</h4>`;
        ...
        const isOpen = !!state.appData.openDbSections[title];
        contentDiv.className = `p-4 space-y-4 border-t border-gray-200 dark:border-gray-700 ${isOpen ? '' : 'hidden'}`;
        ...
        headerBtn.onclick = () => {
            ...
            state.appData.openDbSections[title] = isHidden;
            saveDataDebounced(null);
        };
...
function setupEventListeners() {
    ...
    if (elements.syncTemplatesBtn) {
        elements.syncTemplatesBtn.addEventListener('click', () => {
            showConfirmModal('¿Qué deseas hacer con las plantillas?\n\n- CONFIRMAR: Descargar plantillas actuales (JSON)\n- CANCELAR: Actualizar plantillas desde un archivo (JSON)', () => {
                const dataStr = JSON.stringify(state.appData.templates || [], null, 2);
                const blob = new Blob([dataStr], { type: 'application/json' });
                const url = URL.createObjectURL(blob);
                const a = document.createElement('a'); a.href = url; a.download = `gtn_plantillas_${getFormattedDateForFilename()}.json`;
                document.body.appendChild(a); a.click(); document.body.removeChild(a); URL.revokeObjectURL(url);
                showToast('Plantillas descargadas.', 'success');
            }, 'Sincronizar Plantillas');
            
            const confirmBtn = document.getElementById('confirm-submit-btn');
            const cancelBtn = document.getElementById('confirm-cancel-btn');
            confirmBtn.textContent = 'Descargar';
            cancelBtn.textContent = 'Actualizar';
            
            const customCancelHandler = () => {
                elements.importTemplatesInput.click();
    ...
    if (elements.importTemplatesInput) {
        elements.importTemplatesInput.addEventListener('change', (e) => {
            ...
            reader.onload = (ev) => {
                try {
                    ...
                    if (Array.isArray(parsed)) {
                        state.appData.templates = parsed;
                        saveData(null);
                        renderTemplates();
                        updateSelectionStatus();
                        showToast('Plantillas actualizadas correctamente.', 'success');
                    } else {
                        showToast('El archivo no contiene plantillas válidas.', 'error');
                    }
                } catch (err) {
                    showToast('Error al leer el archivo de plantillas.', 'error');
                }
            };

...
export async function downloadPDF() {
    ...
    try {
        ...
        const renderLine = (lineWords, y, currentFontSize, isLastLine, startX = margin) => {
            doc.setFontSize(currentFontSize);
            ...
            let extraSpacePerGap = spaceWidth; // valor por defecto: alineación izquierda
            // Solo justificar si no es la última línea del párrafo y hay más de 1 palabra
            if (!isLastLine && lineWords.length > 1) {
                const emptySpace = (usableWidth - (startX - margin)) - totalWordsWidth;
                const candidate = emptySpace / (lineWords.length - 1);
                // Evitar justificación extrema por palabras muy largas
                extraSpacePerGap = candidate > spaceWidth * 3 ? spaceWidth : candidate;
            }
            ...
            lineWords.forEach((word, wIdx) => {
                ...
                // Agregar el espacio entre palabras solo si no es la última
